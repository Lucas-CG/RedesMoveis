...
{	
  ...
  void Vehicle::SetupWifi(const WifiHelper &wifi, const YansWifiPhyHelper &phy, const NqosWaveMacHelper &mac)
  {
  ...
  void Vehicle::SetSendWarning(bool sendWarning){
	m_sendWarning = sendWarning;
  }
  
  bool Vehicle::GetSendWarning(){  
	return m_sendWarning;  
  }
  
  void Vehicle::SetStopWarning(bool stopWarning){
	m_stopWarning = stopWarning;
  }
  
  bool Vehicle::GetStopWarning(){  
	return m_stopWarning;  
  }
  
  Ptr<NetDevice> Vehicle::GetNetDevice()
  {
	  return m_device;
  }
}
