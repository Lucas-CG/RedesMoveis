...
private:
  ...
  int       m_numClients; ///< Lucas: number of clients
...
/// Lucas: parâmetros padrão
MeshTest::MeshTest () :
  m_xSize (5),
  m_ySize (5),
  m_step (50.0),
  m_randomStart (0.1),
  /// Lucas: número de clientes
  m_numClients(1),
  m_totalTime (200.0),
...
//  m_stack ("ns3::FlameStack"),
  m_root ("ff:ff:ff:ff:ff:ff") /// Lucas: endereço broadcast (não ativa o modo pró-ativo)
{
}
void
MeshTest::Configure (int argc, char *argv[])
{
  ...
  cmd.AddValue ("step",   "Size of edge in our grid (meters), or spacing between two nodes in the same column or row", m_step);
  ...
  // Lucas: adicionando número de clientes
  cmd.AddValue ("num-clients",  "Number of client nodes (taken from the farthest row to the closest row)", m_numClients);
  cmd.AddValue ("time",  "Simulation time (sec)", m_totalTime);
  cmd.AddValue ("packet-interval",  "Interval between packets in the UDP transmission (sec)", m_packetInterval);
  cmd.AddValue ("packet-size",  "Size of packets in the UDP transmission (bytes)", m_packetSize);
  ...
  cmd.AddValue ("stack",  "Type of protocol stack. ns3::Dot11sStack (default) or ns3::FlameStack (needs to have root as broadcast!)", m_stack);
  cmd.AddValue ("root", "Mac address of root mesh point in HWMP (ff:ff:ff:ff:ff:ff - broadcast - for no root or 00:00:00:00:00:xx - unicast - to set a root) ", m_root);
...
{ 
  ...
  for (int i = 0; i < m_numClients; i++) {
    clientNodes.Add (nodes.Get (m_xSize*m_ySize-1-i));
  }
...
{
//Lucas: UdpEchoServer/Client trocados para UdpServer/Client (registram atraso e número de sequência e o servidor não envia respostas)
  ...
  ApplicationContainer clientApps = client.Install (clientNodes);
